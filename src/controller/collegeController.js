...
const createCollege = async function (req, res) {
    try {
        ...
        if (!name) return res.status(400).send({ msg: "Please enter alphabets only for abbrv and maximum legth should be 10" })
        //<!-----------------Full name Regex------------------------->        
        ...
        if (!fullName) return res.status(400).send({ msg: "Please enter alphabets only for full name of college and maximum length should be 50" })
        //<!-----------------Logolink Regex------------------------->        
        if (!keyValid(data.logoLink)) return res.status(400).send({ status: false, message: "Please enter valid logo link" })
        logoLink = /^https?:\/\/.*\/.*\.(png|gif|webp|jpeg|jpg)\??.*$/gmi
        .test(data.logoLink)
        
        if (!logoLink) return res.status(400).send({ msg: "Please enter valid logo link" })
...
const createIntern = async function (req, res) {
    ...
    try {
        let data = req.body;
        if (!Object.keys(data).length) return res.status(400).send({ status: false, msg: "Write something in body" })
        //---------------name------------------------------
        if (!data.name) return res.status(400).send({ status: false, message: "Please provide with your name" })
        if (!/^[a-zA-Z_ ]+$/.test(data.name)) return res.status(400).send({ msg: "wrong format of name" })
        //--------------email-----------------------------
        ...
        if (!email) return res.status(400).send({ msg: "Please enter valid format of email" })
        let checkEmail = await internModel.find({ email: data.email })
        if (checkEmail.length) return res.status(400).send({ msg: "Email already exists" })
        //--------------mobile--------------------------
        if (!data.mobile) return res.status(400).send({ status: false, msg: "Plese enter mobile number" })
        mobile = /^[8,9]\d{9}$/.test(data.mobile);
        if (!mobile) return res.status(400).send({ msg: "Please enter valid format of mobile" })
        let checkMobile = await internModel.find({ mobile: data.mobile })
        if (checkMobile.length) return res.status(400).send({ msg: "Mobile already exists" })
        //--------------college id----------------------
        ...
        let id = checkCollegeId._id
        let daata = {};
        daata.name = data.name;
        daata.email = data.email;
        daata.mobile = data.mobile
        daata.collegeId = id
        let save = await internModel.create(daata)
        
        var c = {}
        c.isDeleted = save.isDeleted
        c.name = save.name
        c.email = save.email
        c.mobile = save.mobile
        c.collegeId = save.collegeId

        res.status(201).send({ status: true, data: c })
    }
    catch (err) {
        console.log(err)
        res.status(500).send({ msg: err.message })
...
const getCollegeDetails = async (req, res) => {
    ...
    try {
        ...
        if (!collegeName) return res.status(400).send({ status: false, message: "college name not entered" })
        let collegeDetail = await collegeModel.findOne({ name: collegeName, isDeleted: false });
        if (!collegeDetail) return res.status(400).send({ status: false, message: "no details for such college" })
        ...
        if (internDetails.length == 0) return res.status(400).send({ status: false, message: "no interns found" });
        let finalDetail = {};
